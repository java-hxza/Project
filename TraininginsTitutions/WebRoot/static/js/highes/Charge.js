$(function() {
	...
	updateOrder = function() {
		...
		if ($(".LX").attr("name") == 2) {
			...
			$(".feecateId4s").remove();
			$(".integrals").remove();
			var feecateMones = $.trim($(".customCheckes:checked").parent().parent().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().text());
			var num1 = 0;
			for (var i = 0; i < $(".feecateName").length; i++) {
				if ($.trim($(".customCheckes:checked").parent().parent().next().next().next().next().text()).split("/")[num1] == $(".feecateName").eq(i).text()) {
					$(".feecateName").eq(i).next().val(feecateMones.split(",")[num1]);
					num1++;
				}
			}
			$(".date").val($(".customCheckes:checked").parent().parent().next().next().next().text());
			$(".dpMoney").val(parseFloat($(".customCheckes:checked").parent().parent().next().next().next().next().next().next().next().text()) + parseFloat($.trim($(".customCheckes:checked").parent().parent().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().text())));
			...
			$(".feecateIds3").blur(function() {
				dpMoneyFeecateIds = 0;
				for (var i = 0; i < $(".feecateIds3").length; i++) {
					if ($.trim($(".feecateIds3").eq(i).val()) != "") {
						dpMoneyFeecateIds = dpMoneyFeecateIds + parseFloat($.trim($(".feecateIds3").eq(i).val()));
					}
				}
			...
			$(".TiJiao").click(function() {
				...
				var discount = parseFloat($.trim($(".dpMoney").val())) - parseFloat($.trim($(".dpMoneyActivity4").val()));
				var feecateId = "";
				var numbers = 0;
				var feecateMoney = "";
				for (var i = 0; i < $(".feecateIds3").length; i++) {
					if ($.trim($(".feecateIds3").eq(i).val()) != "" && $.trim($(".feecateIds3").eq(i).val()) != 0) {
						numbers++;
						feecateId = $.trim(feecateId + "," + $(".feecateIds3").eq(i).attr("chargeTypeId"));
					}
				}
				for (var i = 0; i < $(".feecateIds3").length; i++) {
					if ($(".feecateIds3").eq(i).val() != "") {
						feecateMoney = feecateMoney + "," + $(".feecateIds3").eq(i).val();
					}
				}
				feecateMoney = feecateMoney.substring(1);
			...
			$(".feecateId4").val($.trim($(".customCheckes:checked").parent().parent().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().text()));
			var num1 = 0;
			var feecateMones = $.trim($(".customCheckes:checked").parent().parent().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().next().text());
